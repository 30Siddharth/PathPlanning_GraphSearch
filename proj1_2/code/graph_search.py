        ...
        # while heap:
            ...
            while neigh and neigh_idx < len(neigh):
                if neigh[neigh_idx] in Cost_dic.keys():                
                    if astar == True:
                        # if 9<= neigh_idx <18:
                        #     d[neigh_idx] = Cost_dic[U] + 1 + np.linalg.norm(np.asarray(goal_index)- np.asarray(neigh[neigh_idx]))
                        # else:
                        #     d[neigh_idx] = Cost_dic[U] + np.linalg.norm(np.asarray(goal_index)- np.asarray(neigh[neigh_idx])) # <--------------<< Cost to go in case of A*
                        # # d[neigh_idx] = Cost_dic[U] + np.sum(abs(np.asarray(goal_index) - np.asarray(neigh_idx)))
                        if 9 <= neigh_idx <18:
                            d[neigh_idx] = Cost_dic[U] + 5
                        elif neigh_idx in [19,20,22,23]:
                            d[neigh_idx] = Cost_dic[U] + 1.5
                        else:
